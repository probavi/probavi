...
// a DuckDB database header records no wall clock (measured). That is weaker than the ordering the
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to duckdb_db_dir, which choose a backup for the "+
...
// the whole of it: nothing in a DuckDB database file records when it was taken.
