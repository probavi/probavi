...
// A DuckDB database header records no wall clock, so file
...
func TestBackupSelectionAccepts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "duckdb_db_dir", params: nil, want: selectNewest,
		},
		{name: "newest", kind: "duckdb_db_dir", params: map[string]string{"select": "newest"}, want: selectNewest},
		{name: "oldest", kind: "duckdb_db_dir", params: map[string]string{"select": "oldest"}, want: selectOldest},
		{name: "random", kind: "duckdb_db_dir", params: map[string]string{"select": "random"}, want: selectRandom},
	}
...
func TestBackupSelectionRefuses(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "duckdb_db_dir", params: map[string]string{"select": "latest"},
			want: "must be newest, oldest or random",
		},
		{
			name: "capitalisation is not a policy either",
			kind: "duckdb_db_dir", params: map[string]string{"select": "Oldest"},
			want: "Oldest is none of them",
		},
		{
			name: "the single-artifact kind chooses nothing",
			kind: "duckdb_db", params: map[string]string{"select": "oldest"},
			want: "kind duckdb_db restores what source.path names",
		},
		{
			name: "nor does an export directory, which is one backup in many files",
			kind: "duckdb_export", params: map[string]string{"select": "random"},
			want: "applies only to duckdb_db_dir",
		},
	}
...
func TestSelectionPicksTheEndsOfTheWindow(t *testing.T) {
	...
	middle := writeCandidate(t, dir, "b.duckdb", 48*time.Hour)
	oldest := writeCandidate(t, dir, "c.duckdb", 72*time.Hour)
	newest := writeCandidate(t, dir, "a.duckdb", 24*time.Hour)
...
func TestSelectionTieBreaksDeterministically(t *testing.T) {
	...
	second := writeCandidate(t, dir, "b.duckdb", 24*time.Hour)
	first := writeCandidate(t, dir, "a.duckdb", 24*time.Hour)
...
func TestRandomCoversTheWindow(t *testing.T) {
	...
	wanted := map[string]bool{
		writeCandidate(t, dir, "a.duckdb", 72*time.Hour): false,
		writeCandidate(t, dir, "b.duckdb", 48*time.Hour): false,
		writeCandidate(t, dir, "c.duckdb", 24*time.Hour): false,
	}
	// A sidecar is newer than every database and is still not a candidate:
	// the DUCK magic predicate runs before the policy, which is what keeps
	// a random draw off a file that is not a backup.
	...
	for range 200 {
		...
		if got == stray {
			t.Fatalf("a random draw reached %s, which holds no DUCK magic", filepath.Base(stray))
		}
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	dir := t.TempDir()
	oldest := writeCandidate(t, dir, "z-oldest.duckdb", 48*time.Hour)
	writeCandidate(t, dir, "a-newest.duckdb", 24*time.Hour)
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	dir := t.TempDir()
	path := writeCandidate(t, dir, "one.duckdb", 24*time.Hour)
