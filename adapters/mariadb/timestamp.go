...
// never make.
//
// mariadb-dump signs its output off with the time it finished. What it
// does not write is an offset: the value is the wall clock of the host
// that ran the dump (measured — the same dump taken under TZ=Asia/Tokyo and
...
const (
	// dumpCompletedPrefix is what mariadb-dump writes as its last line
	// (the same sentence as its mysqldump ancestor, measured). With
