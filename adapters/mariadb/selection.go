...
// single-artifact kind restores what source.path names, and mariadb_backup
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to mariadb_dump_dir, the one kind that chooses a backup "+
				"for the drill: kind %s restores what source.path names", selectParam, kind)
	}
