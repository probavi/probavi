...
func probePayload() any {
	return map[string]any{
		...
		"engine":            map[string]string{"name": "mariadb"},
		...
		"sql_runner": map[string]any{
			...
			"argv": []string{"mariadb", "-h", "127.0.0.1", "-u", "{{user}}", "-D", "{{database}}",
...
func opHealthcheck(ctx context.Context, c *core, payload json.RawMessage) (any, *protoError) {
	...
	if !healthy {
		detail = fmt.Sprintf("mariadb exited %d", val.ExitCode)
	}
	return map[string]any{
...
func awaitEngine(ctx context.Context, c *core, user string) (float64, *protoError) {
	...
	for {
		val, _, _, perr := c.exec(ctx, execArgs{
			Argv:           []string{"mariadb", "-h", "127.0.0.1", "-u", user, "-N", "-B", "-e", "SELECT 1"},
			TimeoutSeconds: 5,
		})
		if perr != nil {
			return 0, perr
...
func ensureDatabase(ctx context.Context, c *core, user, database, charset, collation string) *protoError {
	...
	val, _, stderr, perr := c.exec(ctx, execArgs{
		Argv: []string{"mariadb", "-h", "127.0.0.1", "-u", user, "-N", "-B", "-e", stmt},
	})
	if perr != nil {
		return perr
	}
...
// execRestore loads the dump with the mariadb client reading it on stdin.
// Deliberately not the client-side `source` command the sibling adapter
// uses: `source` is a client extension whose handling has already shifted
// between client generations (the MySQL 9 client hands it to the server,
// which rejects it — the sibling's open defect), while stdin is the
// interface every client version reads the same way. The client stops at
// the first error (no --force): partial restores fail loudly (§5). The
// dump path is adapter-composed, never operator input.
...
{ gzip -dc -- "$1"; echo $? > "$2"; } | tee "$1.fifo" | mariadb -h 127.0.0.1 -u "$3" --database "$4" -B
rc=$?
wait
[ "$rc" = 0 ] || exit 1
[ "$(cat "$2")" = 0 ] || exit 90
...
const restoreScript = `
mariadb -h 127.0.0.1 -u "$2" --database "$3" -B < "$1"
rc=$?
[ "$rc" = 0 ] || exit 1
...
// the load failed. No mariadb client produces these: the scripts normalise
...
func mapScriptExit(exitCode int, stderr []byte, what string) *protoError {
	...
	case incompleteDumpExit:
		return protoErr("source_corrupt", false,
			"%s is not a complete dump: it ends without the line mariadb-dump writes when it "+
...
// mapRestoreFailure classifies mariadb client load failures into protocol
...
// "this is not a usable SQL dump" (both shapes measured against 10.11 and
// 12.3 servers).
func mapRestoreFailure(exitCode int, stderr []byte) *protoError {
	...
	if strings.Contains(line, "ERROR 1064") || strings.Contains(line, `ASCII '\0'`) {
		return protoErr("source_corrupt", false, "mariadb rejected the dump: %s", line)
	}
	return protoErr("restore_failed", false, "mariadb load failed: %s", line)
}
