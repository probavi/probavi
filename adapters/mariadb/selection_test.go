...
// clocks here are the sentence mariadb-dump writes at the end of a dump,
// so "older" is a time the tool actually recorded (see timestamp.go).
...
func TestBackupSelectionAccepts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "mariadb_dump_dir", params: nil, want: selectNewest,
		},
		{name: "newest", kind: "mariadb_dump_dir", params: map[string]string{"select": "newest"}, want: selectNewest},
		{name: "oldest", kind: "mariadb_dump_dir", params: map[string]string{"select": "oldest"}, want: selectOldest},
		{name: "random", kind: "mariadb_dump_dir", params: map[string]string{"select": "random"}, want: selectRandom},
	}
...
func TestBackupSelectionRefuses(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "mariadb_dump_dir", params: map[string]string{"select": "latest"},
			want: "must be newest, oldest or random",
		},
		{
			name: "capitalisation is not a policy either",
			kind: "mariadb_dump_dir", params: map[string]string{"select": "Oldest"},
			want: "Oldest is none of them",
		},
		{
			name: "a single-artifact kind selects nothing",
			kind: "mariadb_dump", params: map[string]string{"select": "oldest"},
			want: "kind mariadb_dump restores what source.path names",
		},
		{
			name: "a physical backup directory selects nothing either",
			kind: "mariadb_backup", params: map[string]string{"select": "random"},
			want: "applies only to mariadb_dump_dir",
		},
	}
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	src, perr := resolveSource(context.Background(), "mariadb_dump_dir", dir,
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource(context.Background(), "mariadb_dump", path,
		map[string]string{selectParam: "oldest"})
	if perr == nil {
