...
// backupversion.go reads which server release series a mariadb-backup
// backup was taken from, out of the backup itself.
//
// A physical backup restores only into its own release series
// (docs/engine-versions.md §5): mariadb-backup output from a 10.11 server
// belongs on a 10.11 server, 11.4 on 11.4. The metadata file the backup
// carries — mariadb_backup_info since 11.0, xtrabackup_info before it,
// the same pair backuptime.go dates the backup from — records the origin
// server as `server_version`. Reading it host-side costs nothing and lets
// the drill refuse an impossible pairing before a byte is transferred,
// with a message that names both sides, instead of surfacing whatever the
// prepare or the restored server prints when the data files belong to a
// different series.

// serverVersionKey is the metadata line naming the server the backup was
// taken from ("11.4.7-MariaDB", "10.11.8-MariaDB-1:10.11.8+maria~ubu2204").
