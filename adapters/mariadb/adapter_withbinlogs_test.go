...
// adapter_withbinlogs_test.go drives the mariadb_backup_with_binlogs kind
...
// serving — because mariadb-binlog produces SQL and a server has to be up to
// accept it.
...
func withBinlogsPayload(dir, pitr string) string {
	...
	return fmt.Sprintf(`{"source":{"kind":"mariadb_backup_with_binlogs","path":%q,`+
...
func binlogHandler(t *testing.T, sequence *[]string, replay *[]string) func(verbCall) (any, *protoError) {
	...
	return func(call verbCall) (any, *protoError) {
		if call.Verb == "put_file" {
			...
			if args.DestPath == "/scratch/probavi-mariadb-backup-binlogs" {
		...
		if len(args.Argv) > 2 && strings.Contains(args.Argv[2], "mariadb-binlog") {
...
func TestProvisionWithBinlogsReplaysAfterTheEngineIsUp(t *testing.T) {
	...
	want := []string{"client-idle", "backup-tool-version", "put_file", "prepare", "restore",
		"start", "client-ready", "put_file-binlogs", "replay"}
	...
	wantTail := "/scratch/probavi-mariadb-backup-binlogs 812 2026-09-25 14:30:00 " +
...
func TestWithBinlogsSourceRefusals(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			payload := fmt.Sprintf(`{"source":{"kind":"mariadb_backup_with_binlogs","path":%q,"params":%s},`+
...
func TestPITRIsRefusedOnEveryOtherKind(t *testing.T) {
	for _, kind := range []string{"mariadb_dump", "mariadb_dump_dir", "mariadb_backup"} {
		t.Run(kind, func(t *testing.T) {
			...
			if f.OK || f.Error.Code != "invalid_request" ||
				!strings.Contains(f.Error.Message, "mariadb_backup_with_binlogs") {
