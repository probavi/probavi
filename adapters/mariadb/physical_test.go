...
func TestResolveMariadbBackupSource(t *testing.T) {
	backup := writeBackupFixture(t)
	src, perr := resolveSource(context.Background(), "mariadb_backup", backup, nil)
	if perr != nil {
	...
	if _, perr := resolveSource(context.Background(), "mariadb_backup", file, nil); perr == nil || perr.Code != "invalid_request" {
		t.Errorf("file as backup dir: %+v, want invalid_request", perr)
	}
	if _, perr := resolveSource(context.Background(), "mariadb_backup", filepath.Join(t.TempDir(), "gone"), nil); perr == nil || perr.Code != "source_not_found" {
		t.Errorf("missing backup dir: %+v, want source_not_found", perr)
	}

	// A directory of files that is not a mariadb-backup backup must be
	// refused before any transfer.
	notBackup := t.TempDir()
	if err := os.WriteFile(filepath.Join(notBackup, "random.dat"), []byte("x"), 0o600); err != nil {
		t.Fatalf("write: %v", err)
	}
	if _, perr := resolveSource(context.Background(), "mariadb_backup", notBackup, nil); perr == nil || perr.Code != "source_corrupt" ||
		!strings.Contains(perr.Message, "mariadb_backup_checkpoints") {
...
func TestBackupWithoutMetadata(t *testing.T) {
	...
	src, perr := resolveSource(context.Background(), "mariadb_backup", backup,
...
// TestModernMetadataNamesAccepted covers the 11.0 rename: a backup whose
// metadata files carry the mariadb_backup_* names (measured on 11.4) must
// resolve and date exactly as a 10.x one does.
func TestModernMetadataNamesAccepted(t *testing.T) {
	backup := filepath.Join(t.TempDir(), "backup")
	for path, content := range map[string]string{
		"mariadb_backup_checkpoints": "backup_type = full-backuped\n",
		"mariadb_backup_info":        "start_time = 2026-08-10 00:50:23\nend_time = 2026-08-10 00:50:25\n",
		"ibdata1":                    "innodb-bytes",
	} {
		full := filepath.Join(backup, path)
		if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
	...
	src, perr := resolveSource(context.Background(), "mariadb_backup", backup,
		map[string]string{backupTimezoneParam: "Asia/Tokyo"})
	if perr != nil {
		t.Fatalf("resolveSource: %+v — the 11.0 metadata names must be accepted", perr)
	}
	if src.createdAt == nil || *src.createdAt != "2026-08-10T00:50:25.000+09:00" {
		t.Errorf("createdAt = %v, want the completion time read from mariadb_backup_info", src.createdAt)
	}
}

func physicalPayload(backup string) string {
	return fmt.Sprintf(`{"source":{"kind":"mariadb_backup","path":%q,"params":{}},"sandbox":{"scratch_dir":"/scratch"},"options":{}}`, backup)
}
...
func physicalHandler(t *testing.T, sequence *[]string) func(verbCall) (any, *protoError) {
	...
	return func(call verbCall) (any, *protoError) {
		if call.Verb == "put_file" {
			...
			if args.DestPath != "/scratch/probavi-mariadb-backup" || args.Mode != "0755" {
		...
		case args.Argv[0] == "mariadb":
			*sequence = append(*sequence, "client-ready")
		...
		case strings.Contains(stmt, "probavi-init.sql"):
...
func TestProvisionPhysicalHappyPath(t *testing.T) {
	backup := writeBackupFixture(t)
	var sequence []string
	line, _, exit := driveOp(t, "provision", physicalPayload(backup), physicalHandler(t, &sequence))
	if exit != 0 {
	...
	want := []string{"client-idle", "backup-tool-version", "put_file", "prepare", "restore", "start", "client-ready"}
...
// noBackupTool simulates an idle sandbox whose image lacks mariadb-backup.
func noBackupTool() func(verbCall) (any, *protoError) {
	call := 0
	return func(verbCall) (any, *protoError) {
		call++
		if call == 1 {
		...
		return errExec(127, "mariadb-backup: not found"), nil
	}
}
...
func TestProvisionPhysicalFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"running engine refused", physicalPayload(backup), alreadyRunning, "invalid_request", "idle sandbox"},
		{"image without mariadb-backup refused", physicalPayload(backup), noBackupTool(), "invalid_request", "lacks mariadb-backup"},
		{"restore failure classified", physicalPayload(backup), restoreFails(t), "restore_failed", "xtrabackup_logfile"},
		{"non-backup directory refused before any verb",
			physicalPayload(t.TempDir()), mustNotBeCalled(t), "source_not_found", "contains no files"},
	}
...
// versionedBackupFixture writes a backup whose metadata names the origin
// server, so the version pre-check has something to read.
func versionedBackupFixture(t *testing.T, serverVersion string) string {
	t.Helper()
	backup := writeBackupFixture(t)
	if err := os.WriteFile(filepath.Join(backup, backupInfoName),
		[]byte("tool_name = mariadb-backup\nserver_version = "+serverVersion+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	return backup
}

// mariadbdAnswering wraps physicalHandler with an answer for the engine
// version probe; an empty out simulates an image whose PATH has no
// mariadbd.
func mariadbdAnswering(t *testing.T, sequence *[]string, out string) func(verbCall) (any, *protoError) {
	...
	return func(call verbCall) (any, *protoError) {
		if call.Verb == "exec" {
			...
			if args.Argv[0] == "mariadbd" {
				*sequence = append(*sequence, "mariadbd-version")
				if out == "" {
					return errExec(127, "mariadbd: not found"), nil
				}
...
// driveVersionedProvision runs a mariadb_backup provision against a backup
...
func driveVersionedProvision(t *testing.T, serverVersion, out string) (finalResponse, string) {
	...
	var sequence []string
	line, _, _ := driveOp(t, "provision", physicalPayload(backup),
		mariadbdAnswering(t, &sequence, out))
	return parseFinal(t, line), strings.Join(sequence, " | ")
}

func TestPhysicalVersionPrecheckRefusesAMismatch(t *testing.T) {
	f, sequence := driveVersionedProvision(t, "10.11.8-MariaDB",
		"mariadbd  Ver 11.4.7-MariaDB-ubu2404-log for debian-linux-gnu on x86_64 (mariadb.org binary distribution)")
	if f.OK || f.Error.Code != "invalid_request" {
		t.Fatalf("final = %+v, want invalid_request", f)
	}
	for _, want := range []string{"series 10.11", "engine is 11.4", "mariadb:10.11"} {
...
func TestPhysicalVersionPrecheckPasses(t *testing.T) {
	const engine114 = "mariadbd  Ver 11.4.7-MariaDB-ubu2404-log for debian-linux-gnu on x86_64 (mariadb.org binary distribution)"

	t.Run("match proceeds and runs before the transfer", func(t *testing.T) {
		f, sequence := driveVersionedProvision(t, "11.4.2-MariaDB", engine114)
		if !f.OK {
			t.Fatalf("final = %+v", f)
		}
		if !strings.Contains(sequence, "mariadbd-version") {
			t.Fatalf("sequence %q missing the version probe", sequence)
		}
		if strings.Index(sequence, "mariadbd-version") > strings.Index(sequence, "put_file") {
	...
	t.Run("unanswerable engine version skips the check", func(t *testing.T) {
		f, _ := driveVersionedProvision(t, "10.11.8-MariaDB", "")
		if !f.OK {
	...
	t.Run("backup without metadata skips the check", func(t *testing.T) {
		backup := writeBackupFixture(t) // no metadata file: no version to read
		var sequence []string
		line, _, _ := driveOp(t, "provision", physicalPayload(backup), physicalHandler(t, &sequence))
		if f := parseFinal(t, line); !f.OK {
			t.Fatalf("final = %+v", f)
		}
		if strings.Contains(strings.Join(sequence, " | "), "mariadbd-version") {
