...
//
// The coordinate comes from the backup itself. `mariadb-backup --backup`
// writes a binlog-info file naming the log file and the position the
...
// The file has two names across the supported releases (below).
//
...
func readBinlogStart(backupDir string) (binlogStart, *protoError) {
	name, raw, perr := readBinlogInfo(backupDir)
	if perr != nil {
		return binlogStart{}, perr
	}
	...
	if len(fields) < 2 || fields[0] == "" {
		return binlogStart{}, protoErr("source_corrupt", false,
			"%s does not name a log file and position: %q", name, firstTextLine(raw))
	}
	if fields[0] != filepath.Base(fields[0]) {
		return binlogStart{}, protoErr("source_corrupt", false,
			"%s names %q, which is a path rather than a log file name", name, fields[0])
	}
	pos, err := strconv.ParseInt(strings.TrimSpace(fields[1]), 10, 64)
	if err != nil || pos < 0 {
		return binlogStart{}, protoErr("source_corrupt", false,
			"%s does not carry a readable position: %q", name, fields[1])
	}
	return binlogStart{file: fields[0], position: pos}, nil
}

// readBinlogInfo finds the coordinate file under whichever of its two
// names this backup's release wrote, and returns that name so every
// diagnostic quotes the file the operator will actually look for.
//
// Its absence is refused rather than worked around: without it the replay
// would have to start at the beginning of a log and re-apply writes the
// full already contains, and "restored twice" is not a thing a drill may
// quietly do.
func readBinlogInfo(backupDir string) (string, []byte, *protoError) {
	for _, name := range binlogInfoNames {
		raw, err := os.ReadFile(filepath.Clean(filepath.Join(backupDir, name)))
		switch {
		case err == nil:
			return name, raw, nil
	...
	return "", nil, protoErr("source_corrupt", false,
		"the backup carries no %s, so there is no position to replay binary logs from: "+
			...
			"drill cannot rest on it", strings.Join(binlogInfoNames, " or "))
}
...
func binlogFilesFrom(dir string, start binlogStart) ([]string, *protoError) {
	...
	if !ok {
		return nil, protoErr("source_corrupt", false,
			"the backup's binlog-info names %q, which is not a binary log file name "+
				"(expected <base>.<ordinal>)", start.file)
	}
...
// the form mariadb-binlog takes, and "" when the drill asked for no target.
//
...
// mariadb-binlog that dies half way through a log leaves the pipeline's
// exit status to the client, which happily reports success for the
// fragment it did receive — a drill that proved a partial recovery and
// said nothing.
//
...
cd "$dir"
if [ -n "$stop" ]; then
  TZ=UTC mariadb-binlog --start-position="$pos" --stop-datetime="$stop" -- "$@" |
    mariadb -h 127.0.0.1 -u "$user"
else
  TZ=UTC mariadb-binlog --start-position="$pos" -- "$@" | mariadb -h 127.0.0.1 -u "$user"
fi`
