...
//
// arangodb_dump_dir restored the newest member, and nothing else was on offer. A
...
// artifact records nothing about itself, and here the instant a dump's own dump.json records — so
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to arangodb_dump_dir, which chooses a backup for the drill: "+
