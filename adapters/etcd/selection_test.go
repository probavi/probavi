...
// An etcd snapshot records no time of its own, so file
// modification time is the whole of the ordering (see selection.go).

// writeCandidate writes a snapshot with a modification time in the past, far
...
func writeCandidate(t *testing.T, dir, name string, back time.Duration) string {
	t.Helper()
	return snapshotAt(t, dir, name, time.Now().Add(-back))
}
...
func chooseIn(t *testing.T, dir string, policy selectPolicy) string {
	t.Helper()
	got, perr := chosenSnapshotIn(context.Background(), dir, policy)
	if perr != nil {
		t.Fatalf("chosenSnapshotIn: %+v", perr)
	}
	return got
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "etcd_snapshot_dir", params: nil, want: selectNewest,
		},
		{name: "newest", kind: "etcd_snapshot_dir", params: map[string]string{"select": "newest"}, want: selectNewest},
		{name: "oldest", kind: "etcd_snapshot_dir", params: map[string]string{"select": "oldest"}, want: selectOldest},
		{name: "random", kind: "etcd_snapshot_dir", params: map[string]string{"select": "random"}, want: selectRandom},
	}
...
func TestBackupSelectionRefuses(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "etcd_snapshot_dir", params: map[string]string{"select": "latest"},
			want: "must be newest, oldest or random",
		},
		{
			name: "capitalisation is not a policy either",
			kind: "etcd_snapshot_dir", params: map[string]string{"select": "Oldest"},
			want: "Oldest is none of them",
		},
		{
			name: "the single-artifact kind chooses nothing",
			kind: "etcd_snapshot", params: map[string]string{"select": "oldest"},
			want: "kind etcd_snapshot restores what source.path names",
		},
	}
...
func TestSelectionPicksTheEndsOfTheWindow(t *testing.T) {
	...
	middle := writeCandidate(t, dir, "b.snap", 48*time.Hour)
	oldest := writeCandidate(t, dir, "c.snap", 72*time.Hour)
	newest := writeCandidate(t, dir, "a.snap", 24*time.Hour)
...
func TestSelectionTieBreaksDeterministically(t *testing.T) {
	...
	second := writeCandidate(t, dir, "b.snap", 24*time.Hour)
	first := writeCandidate(t, dir, "a.snap", 24*time.Hour)
...
func TestRandomCoversTheWindow(t *testing.T) {
	...
	wanted := map[string]bool{
		writeCandidate(t, dir, "a.snap", 72*time.Hour): false,
		writeCandidate(t, dir, "b.snap", 48*time.Hour): false,
		writeCandidate(t, dir, "c.snap", 24*time.Hour): false,
	}
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	dir := t.TempDir()
	oldest := writeCandidate(t, dir, "z-oldest.snap", 48*time.Hour)
	writeCandidate(t, dir, "a-newest.snap", 24*time.Hour)
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	dir := t.TempDir()
	path := writeCandidate(t, dir, "one.snap", 24*time.Hour)

	_, perr := resolveSource(context.Background(), "etcd_snapshot", path,
		map[string]string{selectParam: "oldest"})
	if perr == nil {
