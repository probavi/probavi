...
//
// The directory kind here restored the newest member, and nothing else
...
// an etcd snapshot carries no timestamp of its own (measured). That is weaker than the ordering the
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to etcd_snapshot_dir, which choose a backup for the "+
...
// the whole of it: nothing in an etcd snapshot records when it was taken.
