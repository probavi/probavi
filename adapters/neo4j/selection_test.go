...
// neo4j_dump_dir restores. File time is the only fact this adapter has to
...
// writeCandidate lays down one candidate dump archive, backdated.
func writeCandidate(t *testing.T, base, name string, age time.Duration) string {
	t.Helper()
	return writeAgedIn(t, base, name, age)
}

// chooseIn names chooseDumpIn the way the shared tests below call it.
func chooseIn(dir string, policy selectPolicy) (string, *protoError) {
	return chooseDumpIn(context.Background(), dir, policy)
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("neo4j_dump_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("neo4j_dump_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"neo4j_dump"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("neo4j_dump_dir", map[string]string{selectParam: "latest"})
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource(context.Background(), "neo4j_dump_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource(context.Background(), "neo4j_dump", newest,
