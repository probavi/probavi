...
//
// neo4j_dump_dir restored the newest member, and nothing else was on offer. A
...
// a `neo4j-admin database dump` archive carries no timestamp
// of its own (measured). That is weaker than the ordering the postgres and
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to neo4j_dump_dir, which chooses a backup for the drill: "+
