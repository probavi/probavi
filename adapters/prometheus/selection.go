...
//
// prometheus_snapshot_dir restored the newest member, and nothing else was on offer. A
...
// artifact records nothing about itself, and here the newest instant a snapshot's own blocks cover — so
...
//
// A candidate on this kind is a **snapshot directory**, not a file, and it is
...
func selectsAMember(kind string) bool {
	return kind == "prometheus_snapshot_dir"
}
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to prometheus_snapshot_dir, which chooses a backup for the drill: "+
...
// calls for.
func pick(candidates []snapshotCandidate, policy selectPolicy) snapshotCandidate {
	if policy == selectRandom {
		pool := datable(candidates)
		return pool[randomIndex(len(pool))]
	}
	wins := snapshotCandidate.beats
	if policy == selectOldest {
		wins = snapshotCandidate.precedes
	}
	best := candidates[0]
...
// was there at all.
func datable(candidates []snapshotCandidate) []snapshotCandidate {
