...
// selection_test.go covers the policy that decides which snapshot
// prometheus_snapshot_dir restores. A candidate here is a subdirectory dated from what
// it states about itself, so the fixtures state their instants and no test
// leans on a file time to mean "newer".

const (
	windowOldest = maxAug2026 - 7_200_000
	windowMiddle = maxAug2026 - 3_600_000
	windowNewest = maxAug2026
)

// writeCandidate lays down one candidate snapshot whose own blocks cover
// up to maxTime.
func writeCandidate(t *testing.T, base, name string, maxTime int64) string {
	t.Helper()
	return writeSnapshot(t, filepath.Join(base, name), maxTime-1_800_000, maxTime)
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("prometheus_snapshot_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("prometheus_snapshot_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"prometheus_snapshot", "prometheus_snapshot_tar"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("prometheus_snapshot_dir", map[string]string{selectParam: "latest"})
...
func TestSelectionPicksTheEndsOfTheWindow(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got, perr := chooseSnapshotIn(base, tc.policy)
		if perr != nil {
...
func TestOldestKeepsTheDatednessRule(t *testing.T) {
	...
	got, perr := chooseSnapshotIn(base, selectOldest)
	if perr != nil {
		t.Fatalf("chooseSnapshotIn: %+v", perr)
	}
	if got != dated {
...
func TestOldestFallsBackTheSameWayNewestDoes(t *testing.T) {
	...
	if got, _ := chooseSnapshotIn(base, selectOldest); got != older {
		t.Errorf("oldest picked %s, want %s", filepath.Base(got), filepath.Base(older))
	}
	if got, _ := chooseSnapshotIn(base, selectNewest); got != newer {
...
func TestPrecedesIsNotTheNegationOfBeats(t *testing.T) {
	...
	dated := snapshotCandidate{name: "dated", maxTimeMs: 1, mtime: at}
	undated := snapshotCandidate{name: "undated", mtime: at}
...
func TestSelectionTieBreaksDeterministically(t *testing.T) {
	...
	if got, _ := chooseSnapshotIn(base, selectNewest); got != z {
		t.Errorf("newest broke the tie at %s, want %s", filepath.Base(got), filepath.Base(z))
	}
	if got, _ := chooseSnapshotIn(base, selectOldest); got != a {
...
func TestRandomCoversTheWindowAndNothingElse(t *testing.T) {
	...
	seen := map[string]int{}
	for range 200 {
		got, perr := chooseSnapshotIn(base, selectRandom)
		if perr != nil {
			t.Fatalf("chooseSnapshotIn: %+v", perr)
		}
		seen[got]++
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource("prometheus_snapshot_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource("prometheus_snapshot", newest, map[string]string{selectParam: "oldest"})
	if perr == nil || perr.Code != "invalid_request" {
