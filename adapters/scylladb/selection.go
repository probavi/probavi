...
//
// scylladb_snapshot_dir restored the newest member, and nothing else was on offer. A
...
func selectsAMember(kind string) bool {
	return kind == "scylladb_snapshot_dir"
}
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to scylladb_snapshot_dir, which chooses a backup for the drill: "+
