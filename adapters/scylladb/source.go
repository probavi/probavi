...
//
//	scylladb_snapshot_tar — path is one tar archive (plain or gzip) of a
//	                        collected snapshot, keyspaces at the root or
//	                        under one wrapping directory
//	scylladb_snapshot     — path is one collected snapshot tree:
//	                        <keyspace>/<table>/ holding each table's
//	                        snapshots/<tag>/ contents
//	scylladb_snapshot_dir — path is a directory of such trees; the one
//	                        whose own manifests claim the newest instant
//	                        is restored
//
// A snapshot's manifest.json states when it was taken, in epoch seconds
...
func resolveSource(kind, path string) (*resolvedSource, *protoError) {
	switch kind {
	case "scylladb_snapshot_tar":
		return resolveTar(path)
	case "scylladb_snapshot":
		return resolveTree(path)
	case "scylladb_snapshot_dir":
	...
	default:
		return nil, protoErr("unsupported_source", false,
			"unsupported source kind: %s (supported: scylladb_snapshot_tar, scylladb_snapshot, "+
				"scylladb_snapshot_dir)", kind)
	}
}
...
// tar-shaped: the sandbox extraction is then the authority.
func resolveTar(path string) (*resolvedSource, *protoError) {
	...
	case info.IsDir():
		return nil, protoErr("invalid_request", false,
			"source path %s is a directory; use kind scylladb_snapshot for a collected snapshot "+
				"tree, or scylladb_snapshot_dir for a directory of them", path)
	}
...
func resolveTree(dir string) (*resolvedSource, *protoError) {
	...
	case !info.IsDir():
		return nil, protoErr("invalid_request", false,
			"source path %s is a file; use kind scylladb_snapshot_tar for tar archives", dir)
	}
...
// one that cannot, and the chosen tree still faces every single-tree
// gate, so a broken candidate that wins the ranking is refused by name.
...
// instant is epoch seconds, so the offset is literal Z.
...
// manifest.json states its instant as epoch seconds (measured), which
// carries no zone to resolve, so this adapter only refuses the
// declaration: an operator who wrote it expects an accuracy the artifact
// delivers on its own, and silence would leave them believing it did
// something.
...
func rejectBackupTimezone(params map[string]string) *protoError {
	...
	return protoErr("invalid_request", false,
		"source.params.%s has no effect for this adapter: a snapshot's manifest.json states its "+
			"instant in epoch seconds, so backup.created_at is exact without a declared zone — "+
