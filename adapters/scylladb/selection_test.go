...
// scylladb_snapshot_dir restores. A candidate here is a subdirectory dated from what
// it states about itself, so the fixtures state their instants and no test
// leans on a file time to mean "newer".

const (
	windowOldest = int64(1_786_000_000)
	windowMiddle = int64(1_786_086_400)
	windowNewest = int64(1_786_172_800)
)

// writeCandidate lays down one candidate snapshot tree claiming when it
// was taken, in the epoch seconds this engine's manifest records.
func writeCandidate(t *testing.T, base, name string, createdAt int64) string {
	t.Helper()
	dir := filepath.Join(base, name)
	writeSnapshot(t, dir, snapshotTable{
		keyspace: "shop", table: "orders", createdAt: createdAt, sstables: 1,
	})
	return dir
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("scylladb_snapshot_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("scylladb_snapshot_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"scylladb_snapshot", "scylladb_snapshot_tar"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("scylladb_snapshot_dir", map[string]string{selectParam: "latest"})
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource("scylladb_snapshot_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource("scylladb_snapshot", newest, map[string]string{selectParam: "oldest"})
	if perr == nil || perr.Code != "invalid_request" {
