...
// firebird_gbak_dir restores. File time is the only fact this adapter has to
...
// writeCandidate lays down one candidate gbak backup, backdated.
func writeCandidate(t *testing.T, base, name string, age time.Duration) string {
	t.Helper()
	return writeAgedIn(t, base, name, age)
}

// chooseIn names chooseBackupIn the way the shared tests below call it.
func chooseIn(dir string, policy selectPolicy) (string, *protoError) {
	return chooseBackupIn(context.Background(), dir, policy)
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("firebird_gbak_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("firebird_gbak_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"firebird_gbak"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("firebird_gbak_dir", map[string]string{selectParam: "latest"})
...
func TestSelectionPicksTheEndsOfTheWindow(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got, perr := chooseIn(base, tc.policy)
		if perr != nil {
...
func TestSelectionTieBreaksDeterministically(t *testing.T) {
	...
	if got, _ := chooseIn(base, selectNewest); got != z {
		t.Errorf("newest broke the tie at %s, want %s", filepath.Base(got), filepath.Base(z))
	}
	if got, _ := chooseIn(base, selectOldest); got != a {
...
func TestRandomCoversTheWindow(t *testing.T) {
	...
	seen := map[string]int{}
	for range 200 {
		got, perr := chooseIn(base, selectRandom)
		if perr != nil {
...
func TestTheSettleCheckAppliesUnderEveryPolicy(t *testing.T) {
	...
	_, perr := chooseIn(base, selectOldest)
	stop()
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource(context.Background(), "firebird_gbak_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource(context.Background(), "firebird_gbak", newest,
