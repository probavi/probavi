...
//
// firebird_gbak_dir restored the newest member, and nothing else was on offer. A
...
// a gbak backup's header carries the wall clock of the host
// that took it, with no offset, so it is not an instant this adapter may
// rank two artifacts by — reporting it needs a declared zone, and ranking
// must not depend on one being declared. That is weaker than the ordering the postgres and
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to firebird_gbak_dir, which chooses a backup for the drill: "+
