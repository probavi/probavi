...
// TestDeclaredStatementsCarryNoSpace is the constraint that shaped them.
...
// Every declared statement must therefore be one word per argument, which
// is why the count reads return#redis.call(...) and the message has
// underscores.
func TestDeclaredStatementsCarryNoSpace(t *testing.T) {
	...
	for kind, declared := range got.Checks {
		for _, word := range strings.Fields(declared["statement"]) {
			if strings.ContainsAny(word, "\"'`") && strings.Contains(word, " ") {
				t.Errorf("checks[%q] has a quoted phrase; the runner splits on whitespace", kind)
			}
		}
		// EVAL <script> <numkeys> <pattern>: four words exactly, because
		// each has to survive as its own argument.
		if n := len(strings.Fields(declared["statement"])); n != 4 {
			t.Errorf("checks[%q] = %q splits into %d words, want exactly 4 (EVAL script numkeys pattern)",
				kind, declared["statement"], n)
		}
		if !strings.HasSuffix(declared["statement"], "{{table}}:*") {
			t.Errorf("checks[%q] = %q, want it to ask about the keys under <table>:",
				kind, declared["statement"])
		}
	}
	if _, declared := got.Checks["freshness"]; declared {
		t.Error("freshness is declared, but Redis dates nothing per key for it to read")
	}
