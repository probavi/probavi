...
`<table>:`, which is the naming convention Redis itself documents. Neither
applied before, because the core composed SQL and Redis has none.
...
Three things worth knowing, each measured on 7.2.15:

- **`KEYS` reads the whole keyspace**, and on a large restore this is the
  expensive check in the set. It is the only form the runner can carry:
  `SCAN` needs a loop, and a loop needs spaces — see below.
- `row_count` answers **0** for a prefix holding nothing rather than
  ...
  bound, which is why `table_exists` cannot be the same statement: it
  asserts the first key instead and fails with its own message when there
  is none.
...
a space arrives as several arguments and the engine answers `ERR value is
not an integer or out of range` (measured). That is why the count reads
`return#redis.call(...)` and the failure message is
