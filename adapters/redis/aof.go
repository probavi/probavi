...
// aof.go reads what a Redis 7+ append-only directory states about
// itself. Since 7.0 the append-only state is a directory (the server's
// appenddirname, "appendonlydir" by default) holding a text manifest
// plus the files it names: one base (an RDB by default) and incremental
// AOF segments. The manifest is the authority on what the artifact must
// contain, which gives this adapter the gate AOF backups need most: a
// copy taken mid-rewrite loses members, and a manifest naming a file
// the backup does not hold is positive evidence of exactly that —
// refused by name, before a byte reaches the sandbox.
//
// The base file, when it is an RDB, carries the same header the rdb
// kinds read (rdbmeta.go): redis-ver feeds the engine-version pre-check
// and the valkey markers feed the dialect fence. Its ctime is
...
func readAOFDir(path string) ([]os.DirEntry, *protoError) {
	...
	case !info.IsDir():
		return nil, protoErr("invalid_request", false,
			"source path %s is a file; redis_aof restores the Redis 7+ append-only directory "+
				"(manifest, base, incremental segments) — a pre-7.0 single-file AOF is not supported, "+
				"see the adapter README", path)
	}
...
func findManifest(dir string, entries []os.DirEntry) (string, *protoError) {
	...
	return "", protoErr("source_corrupt", false,
		"backup directory %s holds no .manifest file — not a Redis 7+ append-only directory "+
