...
func resolveFile(path string) (*resolvedSource, *protoError) {
	...
	// stays the authority on whether the file restores.
...
// records about itself. Files without the RDB magic are skipped as
// non-candidates (checksum sidecars, README files); if nothing qualifies,
// the skipped names are counted so the refusal says what was passed over.
