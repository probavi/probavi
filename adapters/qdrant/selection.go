...
//
// qdrant_snapshot_dir and qdrant_full_snapshot_dir restored the newest member, and nothing else was on
...
//
// **What this adapter orders by is file modification time**,
// because no Qdrant snapshot records when it was taken (measured). That is weaker than the ordering the postgres
// and cassandra adapters apply, and the difference is worth stating rather
...
// single-artifact kinds restore what source.path names, so a selection
// policy there would mean nothing, and accepting the word for them would
// be the wrong kind of convenience.
func selectsAMember(kind string) bool {
	switch kind {
	case "qdrant_snapshot_dir", "qdrant_full_snapshot_dir":
		return true
	}
	return false
}
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to qdrant_snapshot_dir and qdrant_full_snapshot_dir, which choose a backup for the drill: "+
...
// dirCandidate is one snapshot a directory source could restore.
// File time is the whole of it, for the reason given at the top of
// this file.
...
// beats orders two candidates for the newest policy: newer snapshot,
...
// invert. With File time as the only fact, there is nothing
// asymmetric left.
