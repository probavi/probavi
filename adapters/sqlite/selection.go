...
//
// The directory kinds here restored the newest member, and nothing else
...
// knowledge — a dump trailer in one adapter, an archive header in
...
// wall clock (measured). That is weaker than the ordering the
// postgres and mysql adapters apply, and the difference is worth stating
// rather than leaving to be discovered: copying a backup into the
// directory (cp without -p, an object-store download, an rsync without
// -t) gives it a fresh modification time, so a stale artifact looks like
// the newest thing there — and, under oldest, a freshly copied old backup
// stops looking like the oldest. The ordering is honest about what it
// has; the README says so too.
//
...
func selectsAMember(kind string) bool {
	switch kind {
	case "sqlite_db_dir", "sqlite_dump_dir":
		return true
	}
	return false
}
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to sqlite_db_dir and sqlite_dump_dir, which choose a backup for the "+
...
// dirCandidate is one file a directory source could restore. File time is
// the whole of it: nothing in a SQLite database file or a .dump script records when it was taken.
...
// postgres and mysql adapters it is not: those rank a backup carrying its
// own recorded time above one that does not, and that rule cannot invert.
// With file time as the only fact, there is nothing asymmetric left.
