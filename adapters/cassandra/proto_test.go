...
// every line it reads — so an adapter answering with a constant would fail
// the moment negotiation chose anything else.
...
// TestProbeDeclaresTheStatementCQLCanBeAsked: table_exists is the one
// built-in CQL cannot be asked in the core's words, so the adapter
// declares the one it can (issue #277). No identifier is declared, and
// that is not an omission — CQL quotes identifiers exactly as the core
// does, and an adapter says only what is true of its engine.
func TestProbeDeclaresTheStatementCQLCanBeAsked(t *testing.T) {
	...
	if got.Checks["table_exists"]["statement"] != "DESCRIBE TABLE {{table}}" {
		t.Errorf("checks = %v, want table_exists declared as DESCRIBE TABLE", got.Checks)
	}
	if len(got.Checks) != 1 {
		t.Errorf("checks = %v, want only the one built-in CQL cannot express", got.Checks)
	}
	if got.Identifier != nil {
		t.Errorf("identifier = %v, want none: CQL quotes as the core does", got.Identifier)
	}
}

// TestRunnerNoLongerRewritesStatements: the declaration deleted the
// recognition. A check of the operator's own now reaches cqlsh byte for
// byte, which is what the README always promised.
func TestRunnerNoLongerRewritesStatements(t *testing.T) {
	for _, gone := range []string{"WHERE 1=0", "sed -E", "grep -Eq", "probe="} {
		if strings.Contains(runnerScript, gone) {
			t.Errorf("runnerScript still carries %q: the adapter is recognising the core's CQL again", gone)
		}
	}
	if !strings.Contains(runnerScript, "cqlsh --no-color") {
		t.Error("runnerScript lost the runner itself")
	}
}
