...
//
// cassandra_snapshot_dir restored the newest member, and nothing else was on offer. A
...
// artifact records nothing about itself, and here the instant a snapshot's own manifest.json records — so
...
//
// A candidate on this kind is a **snapshot tree**, not a file, and it is
...
func selectsAMember(kind string) bool {
	return kind == "cassandra_snapshot_dir"
}
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to cassandra_snapshot_dir, which chooses a backup for the drill: "+
...
// calls for.
func pick(candidates []treeCandidate, policy selectPolicy) treeCandidate {
	if policy == selectRandom {
		pool := datable(candidates)
		return pool[randomIndex(len(pool))]
	}
	wins := treeCandidate.beats
	if policy == selectOldest {
		wins = treeCandidate.precedes
	}
	best := candidates[0]
...
// was there at all.
func datable(candidates []treeCandidate) []treeCandidate {
	dated := make([]treeCandidate, 0, len(candidates))
