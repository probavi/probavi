...
| `cassandra_snapshot_dir` | a directory of such trees; `source.params.select` picks one — newest by the instant each tree's own manifests claim (the default), oldest, or random |
...
`select` on a kind that chooses nothing — `cassandra_snapshot` and
`cassandra_snapshot_tar` — is **refused** rather than ignored, the same
