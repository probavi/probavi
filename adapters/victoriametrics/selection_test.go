...
// victoriametrics_backup_dir restores. A candidate here is a subdirectory dated from what
...
// writeCandidate lays down one candidate backup claiming when it was
// taken.
func writeCandidate(t *testing.T, base, name, createdAt string) string {
	t.Helper()
	return writeBackup(t, filepath.Join(base, name), createdAt, onePartition())
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("victoriametrics_backup_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("victoriametrics_backup_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"victoriametrics_backup", "victoriametrics_backup_tar"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("victoriametrics_backup_dir", map[string]string{selectParam: "latest"})
...
func TestOldestKeepsTheDatednessRule(t *testing.T) {
	...
	got, perr := chooseBackupIn(base, selectOldest)
	if perr != nil {
...
func TestOldestFallsBackTheSameWayNewestDoes(t *testing.T) {
	...
	if got, _ := chooseBackupIn(base, selectOldest); got != older {
		t.Errorf("oldest picked %s, want %s", filepath.Base(got), filepath.Base(older))
	}
	if got, _ := chooseBackupIn(base, selectNewest); got != newer {
...
func TestPrecedesIsNotTheNegationOfBeats(t *testing.T) {
	...
	dated := backupCandidate{name: "dated", createdAtMs: 1, mtime: at}
	undated := backupCandidate{name: "undated", mtime: at}
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource("victoriametrics_backup_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource("victoriametrics_backup", newest, map[string]string{selectParam: "oldest"})
	if perr == nil || perr.Code != "invalid_request" {
