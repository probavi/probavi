...
// h2_db_dir restores. File time is the only fact this adapter has to
...
// writeCandidate lays down one candidate database file, backdated.
func writeCandidate(t *testing.T, base, name string, age time.Duration) string {
	t.Helper()
	return writeAgedIn(t, base, name, age)
}

// chooseCandidate is chooseIn bound to the database-file predicate, the
// way the h2_db_dir kind itself calls it.
func chooseCandidate(dir string, policy selectPolicy) (string, *protoError) {
	return chooseIn(context.Background(), dir, policy, candidateDatabase)
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	for _, kind := range []string{"h2_db_dir", "h2_backup_dir"} {
	...
	for _, kind := range []string{"h2_db", "h2_backup"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("h2_db_dir", map[string]string{selectParam: "latest"})
...
// whole window, and that an artifact of the other kind is still not a candidate.
func TestRandomCoversTheWindow(t *testing.T) {
	base, oldest, middle, newest := selectionWindow(t)
	// A BACKUP TO archive beside the database files: a candidate for
	// h2_backup_dir, and never one for this kind.
	writeArtifact(t, base, "zz-nightly.zip", []byte("PK\x03\x04 an archive"))
	seen := map[string]int{}
	for range 200 {
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource(context.Background(), "h2_db_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource(context.Background(), "h2_db", newest,
