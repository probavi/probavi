...
//
// h2_db_dir and h2_backup_dir restored the newest member, and nothing else was on
...
// because no H2 artifact records when it was taken: neither an
// .mv.db file's header nor a BACKUP TO archive carries a wall clock
// (measured). That is weaker than the ordering the postgres
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to h2_db_dir and h2_backup_dir, which choose a backup for the drill: "+
...
// dirCandidate is one file a directory source could restore.
...
// beats orders two candidates for the newest policy: newer file,
