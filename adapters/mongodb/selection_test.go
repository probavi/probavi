...
// A mongodump archive records no time of its own, so file
// modification time is the whole of the ordering (see selection.go).

// writeCandidate writes an archive with a modification time in the past, far
// enough back that nothing here trips the settle window (settle.go).
func writeCandidate(t *testing.T, dir, name string, back time.Duration) string {
	t.Helper()
	path := filepath.Join(dir, name)
	if err := os.WriteFile(path, []byte("archive bytes of "+name), 0o600); err != nil {
		t.Fatalf("write %s: %v", name, err)
	}
	when := time.Now().Add(-back)
...
func chooseIn(t *testing.T, dir string, policy selectPolicy) string {
	t.Helper()
	got, perr := chosenDumpIn(context.Background(), dir, policy)
	if perr != nil {
...
func TestBackupSelectionAccepts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "mongodump_dir", params: nil, want: selectNewest,
		},
		{name: "newest", kind: "mongodump_dir", params: map[string]string{"select": "newest"}, want: selectNewest},
		{name: "oldest", kind: "mongodump_dir", params: map[string]string{"select": "oldest"}, want: selectOldest},
		{name: "random", kind: "mongodump_dir", params: map[string]string{"select": "random"}, want: selectRandom},
	}
...
func TestBackupSelectionRefuses(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			kind: "mongodump_dir", params: map[string]string{"select": "latest"},
			want: "must be newest, oldest or random",
		},
		{
			name: "capitalisation is not a policy either",
			kind: "mongodump_dir", params: map[string]string{"select": "Oldest"},
			want: "Oldest is none of them",
		},
		{
			name: "the single-artifact kind chooses nothing",
			kind: "mongodump", params: map[string]string{"select": "oldest"},
			want: "kind mongodump restores what source.path names",
		},
		{
			name: "nor do the two kinds that replay the same archive differently",
			kind: "mongodump_with_oplog", params: map[string]string{"select": "random"},
			want: "applies only to mongodump_dir",
		},
	}
...
func TestSelectionPicksTheEndsOfTheWindow(t *testing.T) {
	...
	middle := writeCandidate(t, dir, "b.archive", 48*time.Hour)
	oldest := writeCandidate(t, dir, "c.archive", 72*time.Hour)
	newest := writeCandidate(t, dir, "a.archive", 24*time.Hour)
...
func TestSelectionTieBreaksDeterministically(t *testing.T) {
	...
	second := writeCandidate(t, dir, "b.archive", 24*time.Hour)
	first := writeCandidate(t, dir, "a.archive", 24*time.Hour)
...
func TestRandomCoversTheWindow(t *testing.T) {
	...
	wanted := map[string]bool{
		writeCandidate(t, dir, "a.archive", 72*time.Hour): false,
		writeCandidate(t, dir, "b.archive", 48*time.Hour): false,
		writeCandidate(t, dir, "c.archive", 24*time.Hour): false,
	}
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	dir := t.TempDir()
	oldest := writeCandidate(t, dir, "z-oldest.archive", 48*time.Hour)
	writeCandidate(t, dir, "a-newest.archive", 24*time.Hour)
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	dir := t.TempDir()
	path := writeCandidate(t, dir, "one.archive", 24*time.Hour)

	_, perr := resolveSource(context.Background(), "mongodump", path,
		map[string]string{selectParam: "oldest"})
	if perr == nil {
