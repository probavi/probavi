...
// a mongodump archive carries no timestamp of its own (measured). That is weaker than the ordering the
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to mongodump_dir, which choose a backup for the "+
...
// the whole of it: nothing in a mongodump archive records when it was taken.
