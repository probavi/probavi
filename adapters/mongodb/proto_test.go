...
// adapter makes. The core probes at the floor, drives everything after it
// at the highest version both sides declare, and compares the protocol on
// every line it reads.
...
// TestProbeDeclaresTheBuiltinsInMongoshsOwnLanguage. Until this adapter
// declared them, the three generating built-ins did not apply here at
// all: the core composed SQL and MongoDB has none. §6.1.1 says a declared
// statement need not be SQL, and this is what that is for.
func TestProbeDeclaresTheBuiltinsInMongoshsOwnLanguage(t *testing.T) {
	...
	for _, kind := range []string{"table_exists", "row_count", "freshness"} {
		...
		if stmt == "" {
			t.Errorf("checks[%q] is undeclared; the core would compose SQL for an engine that has none", kind)
			continue
		}
		if strings.Contains(stmt, "SELECT") {
			t.Errorf("checks[%q] = %q, which is SQL: this engine has none", kind, stmt)
		}
		if !strings.Contains(stmt, "{{table}}") {
			t.Errorf("checks[%q] = %q, want it to name the collection it asks about", kind, stmt)
		}
	}
	// freshness is the only kind whose parameters include a column, and
	// the statement must use it in both places that decide the answer:
	// what to read and what to order by. Ordering by anything else would
	// report the value in the newest document rather than the maximum.
	fresh := got.Checks["freshness"]["statement"]
	if strings.Count(fresh, "{{column}}") != 3 {
		t.Errorf("freshness = %q, want the column in the projection, the sort and the read", fresh)
	}
	if !strings.Contains(fresh, "-1") {
		t.Errorf("freshness = %q, want a descending sort: the maximum, not the newest document's value", fresh)
	}
	if got.Identifier != nil {
		t.Errorf("identifier = %v, want none: the core's default quoting already yields a JS string literal", got.Identifier)
	}
}
