...
// questdb_checkpoint_dir restores. File time is the only fact this adapter has to
...
// writeCandidate moves one checkpointed data root into the directory under
// test and dates it. Only the root's own directory time ranks, so nothing
// inside it needs backdating.
...
// chooseCandidate names chooseRootIn the way the shared tests below call it.
func chooseCandidate(dir string, policy selectPolicy) (string, *protoError) {
	return chooseRootIn(context.Background(), dir, policy)
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	for _, kind := range []string{"questdb_checkpoint_dir"} {
	...
	for _, kind := range []string{"questdb_checkpoint", "questdb_data"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("questdb_checkpoint_dir", map[string]string{selectParam: "latest"})
...
// whole window, and that a directory that is not a data root is still not a candidate.
func TestRandomCoversTheWindow(t *testing.T) {
	base, oldest, middle, newest := selectionWindow(t)
	// A directory with no db subdirectory: not a QuestDB data root, so
	// never a candidate.
	if err := os.MkdirAll(filepath.Join(base, "zz-not-a-root"), 0o755); err != nil {
		t.Fatal(err)
	}
	seen := map[string]int{}
	for range 200 {
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource(context.Background(), "questdb_checkpoint_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource(context.Background(), "questdb_checkpoint", newest,
