...
//
// A copy job that writes to a temporary name and renames on completion
// never trips this at all — the directory only ever shows finished trees.
// That is the real fix, and the adapter README recommends it. It matters
// more here than for a single-file artifact: a QuestDB data root is
// hundreds of megabytes of preallocated column files even when it holds a
// few hundred rows (328 MB measured for 250), so the copy is never
// instantaneous.
...
//
// A QuestDB backup is a data root, and a directory's own size and mtime say
