...
//
// questdb_checkpoint_dir restored the newest member, and nothing else was on
...
//
// **What this adapter orders by is directory modification time**,
// because a QuestDB data root states nothing about when it was
// copied: _instance is 4 KiB of zeroes on an instance with no configured
// id, and no other file in the artifact carries the instant (measured on
// 10.0.1). That is weaker than the ordering the postgres
...
func selectsAMember(kind string) bool {
	return kind == "questdb_checkpoint_dir"
}
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to questdb_checkpoint_dir, which chooses a backup for the drill: "+
...
// dirCandidate is one data root a directory source could restore.
...
// beats orders two candidates for the newest policy: newer data root,
...
// invert. With Directory time as the only fact, there is nothing
// asymmetric left.
