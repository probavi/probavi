...
//
// solr_backup_dir restored the newest member, and nothing else was on
...
// another — so selection is adapter behaviour, and params is what the core
// hands an adapter uninterpreted (protocol §6.2). It therefore costs no
// core config key, no protocol version, and no adapter that does not want
// it.
//
// **What this adapter orders by is directory modification time.** A
// candidate here is a subdirectory, as it is in the cassandra and
// prometheus adapters, but unlike those it is not dated from what it
// states about itself: the startTime a backup_N.properties records belongs
// to one collection inside one backup directory, and this adapter reads it
// for backup.created_at rather than for the ranking (source.go). That
// makes the ordering weaker than those adapters', and the difference is
// worth stating rather than leaving to be discovered: copying a backup
// into the directory (cp without -p, an object-store download, an rsync
// without -t) gives it a fresh modification time, so a stale artifact
// looks like the newest thing there — and, under oldest, a freshly copied
// old backup stops looking like the oldest. The ordering is honest about
// what it has; the README says so too.
//
...
// chose the backup under every one of them, so a backup job still writing
// it is still refused (settle.go).
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to solr_backup_dir, which chooses a backup for the "+
...
// dirCandidate is one backup directory a directory source could restore.
// Directory time is the whole of it, for the reason given at the top of
// this file.
...
// beats orders two candidates for the newest policy: newer directory, then
...
// cassandra and prometheus adapters it is not: those rank a candidate
// carrying its own recorded time above one that does not, and that rule
// cannot invert. With directory time as the only fact, there is nothing
// asymmetric left.
...
// pick chooses one candidate under the policy. The slice is never empty: an
// empty directory is reported by the caller, in the words its own kind
// calls for.
