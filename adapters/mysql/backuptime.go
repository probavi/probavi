...
// backuptime.go dates an XtraBackup backup from the backup itself.
//
// XtraBackup writes xtrabackup_info beside the data files, and it records
...
const (
	// xtrabackupInfoName is the metadata file every XtraBackup backup
	// carries; the adapter already refuses a directory without
	// xtrabackup_checkpoints, so this one sitting beside it is the norm.
