...
func TestPhysicalRestoreStartsTheServerWithTheSchedulerOff(t *testing.T) {
	...
	var sequence []string
	_, calls, exit := driveOp(t, "provision", xtrabackupPayload(backup), physicalHandler(t, &sequence))
	if exit != 0 {
	...
	for _, call := range calls {
		...
		if !strings.Contains(script, "mysqld --daemonize") || !strings.Contains(script, "--init-file") {
			continue
		}
		launched = true
