...
// backup directory.
func writeBinlogInfo(t *testing.T, backup, content string) {
	t.Helper()
	if err := os.WriteFile(filepath.Join(backup, binlogInfoFile), []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
}
...
func TestReadBinlogStart(t *testing.T) {
	t.Run("file and position", func(t *testing.T) {
		...
		writeBinlogInfo(t, backup, "binlog.000004\t1234\n")
	...
	t.Run("a gtid set beside them is ignored", func(t *testing.T) {
		backup := writeBackupFixture(t)
		writeBinlogInfo(t, backup, "binlog.000009\t196\t3E11FA47-71CA-11E1-9E33-C80AA9429562:1-5\n")
		start, perr := readBinlogStart(backup)
		if perr != nil {
			t.Fatalf("readBinlogStart: %+v", perr)
		}
	...
	t.Run("refusals", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				backup := writeBackupFixture(t)
				writeBinlogInfo(t, backup, tc.content)
...
// pipefail — without it a mysqlbinlog that dies half way leaves the exit
