...
func TestDirectorySelectionProvesTheOldestBackup(t *testing.T) {
	...
	res, err := runner.Provision(ctx, &adapter.ProvisionRequest{
		Source: adapter.ProvisionSource{
			Kind: "mysqldump_dir", Path: dir, Params: map[string]string{"select": "oldest"},
		},
		Sandbox: adapter.SandboxInfo{ScratchDir: sbx.ScratchDir()},
	}, sbx)
	if err != nil {
...
//
// The sandbox needs no help to be dangerous here: mysql:8.4 and
// percona-server both start with event_scheduler=ON (measured).
...
CREATE EVENT orders_purge ON SCHEDULE EVERY 1 SECOND
  DO DELETE FROM orders WHERE created < NOW() - INTERVAL 3 DAY;`
	mustExec(t, ctx, seed, "mysql", "-h", "127.0.0.1", "-u", "root", "-e", seedSQL)
	mustExec(t, ctx, seed, "mysqldump", "-h", "127.0.0.1", "-u", "root", "--events",
