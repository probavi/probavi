...
// knowledge — a dump header here, an LSN in another adapter, a file time
// where the artifact records nothing about itself — so selection is
// adapter behaviour, and params is what the core hands an adapter
// uninterpreted (protocol §6.2). It therefore costs no core config key,
// no protocol version, and no adapter that does not want it. Promoting it
// to a validated target.source key once several adapters have implemented
// it and the semantics have settled stays open; that order is reversible
...
// single-artifact kinds restore what source.path names, and the two
// physical kinds restore a repository whose own chain decides the rest
// (chain.go, barman.go) — a selection policy there would mean something
// else, and offering the word for both would be the wrong kind of
// convenience.
func selectsAMember(kind string) bool {
	switch kind {
	case "pgdump_dir", "timescaledb_dump_dir",
		"pgdump_with_globals", "timescaledb_dump_with_globals":
		return true
	}
	return false
}
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to the kinds that choose a backup for the drill "+
				"(pgdump_dir, timescaledb_dump_dir, and the with_globals kinds when source.params.dump "+
				"is unset): kind %s restores what source.path names", selectParam, kind)
	}
