...
func TestEndToEndRestoreDrill(t *testing.T) {
	...
	// Without the Dialect line the core composes SQL for an engine that
	// has none, which is what makes it the assertion.
...
func TestAOFDrillReplaysBaseAndTail(t *testing.T) {
	...
	res, err := runner.Provision(ctx, &adapter.ProvisionRequest{
		Source:  adapter.ProvisionSource{Kind: "valkey_aof", Path: fixture},
		Sandbox: adapter.SandboxInfo{ScratchDir: sbx.ScratchDir()},
	}, sbx)
	if err != nil {
		t.Fatalf("provision: %v", err)
...
// TestCorruptAOFVerdict proves valkey-check-aof inside the sandbox is
...
func TestCorruptAOFVerdict(t *testing.T) {
	...
	if err == nil || !errors.As(err, &aerr) || aerr.Code != "source_corrupt" ||
		...
		t.Fatalf("provision error = %v, want source_corrupt from valkey-check-aof", err)
	}
}
...
// half of issue #166. Valkey stores an absolute instant with every
...
func TestExpiredKeysFailTheDrillInsteadOfPassingAsEmpty(t *testing.T) {
	...
	// A valkey sandbox starts in seconds, so unlike the slower engines
	...
	_, err = runner.Provision(ctx, &adapter.ProvisionRequest{
		Source:  adapter.ProvisionSource{Kind: "valkey_rdb", Path: rdb},
		Sandbox: adapter.SandboxInfo{ScratchDir: sbx.ScratchDir()},
	}, sbx)
...
func makeExpiringRDBFixture(t *testing.T, ctx context.Context, provider *docker.Provider,
	...
	seedScript := `set -e
mkdir -p /tmp/seedrdb
valkey-server --daemonize yes --dir /tmp/seedrdb --dbfilename dump.rdb --appendonly no \
...
valkey-cli -e eval "for i=1,100 do redis.call('SET','session:'..i,'v'..i,'PX',10000) end return 1" 0 >/dev/null
held=$(valkey-cli -e dbsize)
...
func makeAOFFixture(t *testing.T, ctx context.Context, provider *docker.Provider, image, destDir string) {
	...
	seedScript := `set -e
mkdir -p /tmp/seedaof
valkey-server --daemonize yes --dir /tmp/seedaof --appendonly yes --save "" --logfile /tmp/seedaof.log
i=0
until valkey-cli -e ping >/dev/null 2>&1; do
...
valkey-cli -e bgrewriteaof >/dev/null
...
# before the child even starts (measured), which would quietly hand the
# drill the pre-rewrite set this test exists not to settle for.
i=0
...
i=0
until [ "$(valkey-cli info persistence | tr -d '\r' | awk -F: '/aof_rewrite_in_progress/{print $2}')" = "0" ]; do
  i=$((i+1)); [ "$i" -gt 60 ] && { echo "rewrite never finished" >&2; exit 1; }
  sleep 1
done
valkey-cli -e set probavi:tail tail-ok >/dev/null
