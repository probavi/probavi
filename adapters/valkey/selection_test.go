...
// valkey_rdb_dir restores. A candidate here is dated from what it states
...
func writeCandidate(t *testing.T, base, name, ctime string) string {
	t.Helper()
	return writeRDB(t, base, name, "8.0.1", ctime)
}
...
func writeUndatedCandidate(t *testing.T, base, name string) string {
	t.Helper()
	return writeRDB(t, base, name, "8.0.1", "")
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("valkey_rdb_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("valkey_rdb_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"valkey_rdb", "valkey_aof"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("valkey_rdb_dir", map[string]string{selectParam: "latest"})
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSourceIn(t, "valkey_rdb_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSourceIn(t, "valkey_rdb", newest, map[string]string{selectParam: "oldest"})
	if perr == nil || perr.Code != "invalid_request" {
