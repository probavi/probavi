...
// manifest naming the given files, plus the files themselves.
...
// healthyManifest is the two-member set a real server writes.
...
// TestResolveAOFDirHealthySet pins what a real set resolves to: the
...
func TestResolveAOFDirRefusals(t *testing.T) {
	...
	tests := []struct {
		...
		{"a file for the kind teaches the directory shape", legacyFile, "invalid_request", "single-file AOF"},
...
// the origin version for the pre-check, the Redis-dialect markers for
// the fence — and no created_at, deliberately, because the base's ctime
// dates the last rewrite rather than the backup.
func TestResolveAOFReadsTheBaseHead(t *testing.T) {
	t.Run("valkey-ver feeds the pre-check, ctime stays unreported", func(t *testing.T) {
		files := map[string]string{
			"appendonly.aof.1.base.rdb": string(rdbFixture(
				[2]string{"valkey-ver", "8.0.10"}, [2]string{"ctime", "1755400000"})),
			"appendonly.aof.1.incr.aof": "*1\r\n$4\r\nPING\r\n",
		}
		...
		if src.valkeyVer != "8.0.10" || src.aof == nil {
			t.Errorf("src = %+v, want the base's valkey-ver", src)
		}
	...
	t.Run("a Redis-saved base is refused by name", func(t *testing.T) {
		files := map[string]string{
	...
	t.Run("a post-fork Redis format floor fences the base too", func(t *testing.T) {
		files := map[string]string{
			"appendonly.aof.1.base.rdb": string(rdbFixtureMagic("REDIS0012")),
			"appendonly.aof.1.incr.aof": "*1\r\n$4\r\nPING\r\n",
		}
		dir := writeAOFDir(t, filepath.Join(t.TempDir(), "aof"), healthyManifest, files)
		_, perr := resolveSource(t.Context(), "valkey_aof", dir)
	...
	t.Run("a plain-text base contributes nothing and refuses nothing", func(t *testing.T) {
		...
		if src.valkeyVer != "" || src.createdAt != nil {
