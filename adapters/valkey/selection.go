...
//
// valkey_rdb_dir restored the newest member, and nothing else was on offer. A
...
// artifact records nothing about itself, and here the save instant an RDB file's own ctime header field records — so
...
//
// A candidate on this kind is dated by what it states about itself rather
// than by a file time a copy would reset, so oldest is exactly as strong
// here as newest already was. That is not true of every adapter: where the
// artifact states nothing, the ordering is only as good as the directory's
// modification times. A candidate that states nothing still ranks below
// every one that does, under both policies — see precedes in source.go for
// why that one rule does not invert.
//
...
func backupSelection(kind string, params map[string]string) (selectPolicy, *protoError) {
	...
	if !selectsAMember(kind) {
		return "", protoErr("invalid_request", false,
			"source.params.%s applies only to valkey_rdb_dir, which chooses a backup for the drill: "+
...
// calls for.
func pick(candidates []rdbCandidate, policy selectPolicy) rdbCandidate {
	if policy == selectRandom {
		pool := datable(candidates)
		return pool[randomIndex(len(pool))]
	}
	wins := rdbCandidate.beats
	if policy == selectOldest {
		wins = rdbCandidate.precedes
	}
	best := candidates[0]
...
// about — and a draw that ignored it would make an artifact the record
// cannot date as likely to be proved as one it can.
func datable(candidates []rdbCandidate) []rdbCandidate {
	dated := make([]rdbCandidate, 0, len(candidates))
