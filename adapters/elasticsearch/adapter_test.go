...
// fixtureList is the engine's answer to the snapshot listing in the
// shape 8.19 writes (measured: version_id is the index version, version
// a rendered range), snap-2 the newest by its own instant but listed
// first — selection must go by the claimed instants, not file order.
const fixtureList = `{"snapshots":[` +
	`{"snapshot":"snap-2","state":"SUCCESS","version_id":8537000,"version":"8.19.9-8.19.20","end_time_in_millis":1700000100000,"indices":["orders"]},` +
	`{"snapshot":"snap-1","state":"SUCCESS","version_id":8537000,"version":"8.19.9-8.19.20","end_time_in_millis":1700000000000,"indices":["orders"]}]}`

const fixtureRestore = `{"snapshot":{"snapshot":"snap-2","indices":["orders"],"shards":{"total":1,"failed":0,"successful":1}}}`

// fixtureCreatedAt is snap-2's own instant rendered.
const fixtureCreatedAt = "2023-11-14T22:15:00.000Z"

const (
	fixtureSettings = `{"persistent":{},"transient":{},"defaults":{"cluster.name":"docker-cluster",` +
		`"data_streams.lifecycle.poll_interval":"876000h"}}`
	fixtureNodes = `{"nodes":{"HUYRiuXwSMyoFGOBH9WnXA":{"index_version":8537000}}}`
)
...
func defaultSimulated() simulated {
	return simulated{
		...
		ilmStop:   outExecDur(`{"acknowledged":true}`, 0.02),
		ilmStatus: outExecDur(`{"operation_mode":"STOPPED"}`, 0.01),
		settings:  outExecDur(fixtureSettings, 0.01),
		version:   outExec(`{"version":{"number":"8.19.20"}}`),
		nodes:     outExec(fixtureNodes),
...
func classifyShellExec(script string) (string, any) {
	switch {
	case strings.Contains(script, "command -v elasticsearch"):
		return "engine", okExec()
	case script == startScript:
		return "start", execValue{ExitCode: 0, DurationSeconds: 0.2}
	case script == rootScript:
		return "locate", outExec("/scratch/probavi-elasticsearch/extract\n")
	}
	return "", nil
}
...
func classifyCurlExec(joined string, sim simulated) (string, any) {
	...
	case strings.Contains(joined, "/_ilm/stop"):
		return "ilm_stop", sim.ilmStop
	case strings.Contains(joined, "/_ilm/status"):
		return "ilm_status", sim.ilmStatus
	case strings.Contains(joined, "/_cluster/settings"):
		return "settings", sim.settings
	case strings.Contains(joined, "/_nodes"):
		return "nodes", sim.nodes
...
// Elasticsearch SQL travels as one argv element into a bash script that
// JSON-encodes it for the SQL API and filters the tsv answer into
// undecorated tab-separated rows (measured shapes in runnerScript's
// comment).
func TestRunnerTemplateShape(t *testing.T) {
	...
	// The script must not reach for tools the verified images lack.
	for _, tool := range []string{"python3", "jq", "tar"} {
		if strings.Contains(runnerScript, tool) {
			t.Errorf("runnerScript uses %s, which the 9.x image does not ship", tool)
		}
	}
}
...
// indexOf returns the position of the first occurrence of label, or -1.
...
func TestProvisionRestoresRepoDir(t *testing.T) {
	...
	line, _, exit := driveOp(t, "provision",
		provisionPayload(t, "elasticsearch_repo", repo, nil),
	...
	// mkdir for the repo dir, one for the tree skeleton, the lifecycle
	// pins, the identity reads, three files transferred, then the API
	// drill.
	want := map[string]int{"engine": 1, "mkdir": 2, "start": 1, "ready": 1, "ilm_stop": 1,
		"ilm_status": 1, "settings": 1, "version": 1, "nodes": 1, "put_file": 3, "register": 1,
...
// TestLifecyclePinsPrecedeTheRepository pins the order the data-lifecycle
// rule needs: both pins are verified after readiness and before the
// repository is registered — nothing the artifact carries has landed
// when the engine is asked to stand still — and the asynchronous ILM
// stop is polled until the node says STOPPED (measured: STOPPING first).
func TestLifecyclePinsPrecedeTheRepository(t *testing.T) {
	repo := writeRepo(t, t.TempDir())
	...
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		func(call verbCall) (any, *protoError) {
			...
			if label == "ilm_status" {
				statusCalls++
				if statusCalls == 1 {
					return outExec(`{"operation_mode":"STOPPING"}`), nil
				}
			}
			return v, nil
		})
	...
	if statusCalls != 2 {
		t.Errorf("ILM status polled %d times, want STOPPING then STOPPED", statusCalls)
	}
	ready, stop, settings, register := indexOf(sequence, "ready"), indexOf(sequence, "ilm_stop"),
		indexOf(sequence, "settings"), indexOf(sequence, "register")
	if !(ready < stop && stop < settings && settings < register) {
		t.Errorf("sequence = %v, want ready < ilm_stop < settings < register", sequence)
	}
	if put := indexOf(sequence, "put_file"); put < settings {
		t.Errorf("sequence = %v, want the pins verified before any transfer", sequence)
	}
}

// TestLifecyclePollNotPinnedIsRefused pins the verification half of the
// data stream lifecycle pin: a node whose poll interval reads anything
// but the launch value is refused before a byte is transferred.
func TestLifecyclePollNotPinnedIsRefused(t *testing.T) {
	repo := writeRepo(t, t.TempDir())
	sim := defaultSimulated()
	sim.settings = outExec(`{"persistent":{"data_streams.lifecycle.poll_interval":"5m"},"transient":{},` +
		`"defaults":{"data_streams.lifecycle.poll_interval":"876000h"}}`)
	var sequence []string
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		provisionHandler(t, &sequence, sim))
	f := parseFinal(t, line)
	if f.OK || f.Error.Code != "invalid_request" || !strings.Contains(f.Error.Message, "5m") ||
		!strings.Contains(f.Error.Message, lifecyclePollInterval) {
		t.Errorf("final = %+v, want invalid_request naming both intervals", f)
	}
...
func TestProvisionUnpacksArchive(t *testing.T) {
	...
	line, calls, exit := driveOp(t, "provision",
		provisionPayload(t, "elasticsearch_repo_zip", path, nil),
		provisionHandler(t, &sequence, defaultSimulated()))
	f := parseFinal(t, line)
	if exit != 0 || !f.OK {
		t.Fatalf("exit=%d final=%+v", exit, f)
	...
	for _, call := range calls {
		...
		if args.DestPath != "/scratch/probavi-elasticsearch/repo.zip" {
...
// TestProvisionSurvivesTheConformanceSandbox pins check 9 of the
// conformance suite as the adapter sees it: random bytes for the first
// declared kind and an exec that answers a bare `1` to everything — every
// positive-evidence gate stays silent and the operation succeeds.
func TestProvisionSurvivesTheConformanceSandbox(t *testing.T) {
	path := filepath.Join(t.TempDir(), "random.bin")
	writeFixtureFile(t, path, []byte(strings.Repeat("\x00\xff", 2048)))
	line, _, exit := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo_zip", path, nil),
		func(call verbCall) (any, *protoError) {
			if call.Verb == "put_file" {
				return putFileValue{BytesCopied: 4096}, nil
			}
			return outExec("1"), nil
		})
	f := parseFinal(t, line)
	if exit != 0 || !f.OK {
		t.Fatalf("exit=%d final=%+v, want success against the simulated sandbox", exit, f)
	}
}
...
func TestProvisionRefusals(t *testing.T) {
	...
	liveZipDir := filepath.Join(base, "livezip")
	if err := os.MkdirAll(filepath.Join(liveZipDir, "nodes", "0"), 0o755); err != nil {
		t.Fatal(err)
	}
	writeFixtureFile(t, filepath.Join(liveZipDir, "nodes", "0", "node.lock"), []byte(""))
	liveZip := treeToZip(t, liveZipDir, filepath.Join(base, "live.zip"), "")
	...
	tests := []struct {
		...
		{"unknown source kind", provisionPayload(t, "elasticsearch_backup", repo, nil), "unsupported_source"},
		{"missing archive", provisionPayload(t, "elasticsearch_repo_zip", filepath.Join(base, "gone.zip"), nil), "source_not_found"},
		{"a directory for the zip kind", provisionPayload(t, "elasticsearch_repo_zip", repo, nil), "invalid_request"},
		{"a raw data-directory copy", provisionPayload(t, "elasticsearch_repo", liveDir, nil), "unsupported_source"},
		{"a raw copy inside a zip", provisionPayload(t, "elasticsearch_repo_zip", liveZip, nil), "unsupported_source"},
		{"a repository listing no snapshots", provisionPayload(t, "elasticsearch_repo", emptyRepo, nil), "source_corrupt"},
		{"backup_timezone has nothing to declare over epoch instants",
			provisionPayload(t, "elasticsearch_repo", repo, map[string]string{"backup_timezone": "UTC"}), "invalid_request"},
		{"malformed payload", `"not an object"`, "invalid_request"},
		{"pitr is not supported",
			`{"source":{"kind":"elasticsearch_repo","path":"` + repo + `"},"pitr":{"target_time":"2026-08-01T00:00:00Z"}}`,
			"invalid_request"},
	}
...
func TestSandboxPreconditions(t *testing.T) {
	...
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		func(call verbCall) (any, *protoError) {
			return errExec(127, "bash: elasticsearch: command not found"), nil
		})
	f := parseFinal(t, line)
...
// naming both index versions and the engine's release.
func TestVersionPrecheckRefusesBeforeTransfer(t *testing.T) {
	index := strings.ReplaceAll(fixtureIndex, `"index_version":8537000`, `"index_version":9111000`)
	...
	if f.OK || f.Error.Code != "invalid_request" ||
		!strings.Contains(f.Error.Message, "9111000") || !strings.Contains(f.Error.Message, "8537000") ||
		!strings.Contains(f.Error.Message, "8.19.20") {
...
func TestEngineListingNothingContradictsTheCensus(t *testing.T) {
	repo := writeRepo(t, t.TempDir())
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		overrideStep(t, "list", outExec(`{"snapshots":[],"total":0,"remaining":0}`)))
	f := parseFinal(t, line)
...
// pairing gate: when neither the metadata nor the listing carried a
// usable index version, the engine's own refusal at restore is surfaced
// as the pairing problem it is — readable only because the API calls
// run curl without -f.
func TestRestoreRefusalMapsToInvalidRequest(t *testing.T) {
	repo := writeRepoWithIndex(t, t.TempDir(), strings.ReplaceAll(fixtureIndex, `"index_version":8537000,`, ""))
	sim := defaultSimulated()
	sim.list = outExec(strings.ReplaceAll(fixtureList, `"version_id":8537000,`, ""))
	sim.restore = outExec(`{"error":{"root_cause":[{"type":"snapshot_restore_exception",` +
		`"reason":"[probavi:snap-2/AGMAv_H2TjKBUWT3YyduGw] the snapshot was created with version ` +
		`[8.19.21-[9111000]] which is higher than the version of this node [8.19.9-8.19.20]"}]},"status":500}`)
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		func(call verbCall) (any, *protoError) {
			if call.Verb == "put_file" {
				return putFileValue{}, nil
	...
	f := parseFinal(t, line)
	if f.OK || f.Error.Code != "invalid_request" ||
		!strings.Contains(f.Error.Message, "which is higher than the version of this node") {
...
// damaged (measured: 4 of 4), so the verdict is read from the shard
// counts.
func TestShardFailuresAreTheVerdict(t *testing.T) {
	repo := writeRepo(t, t.TempDir())
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		overrideStep(t, "restore",
			outExec(`{"snapshot":{"snapshot":"snap-2","indices":["orders"],"shards":{"total":2,"failed":1,"successful":1}}}`)))
	f := parseFinal(t, line)
...
func TestStartFailureCarriesTheNodesOwnLine(t *testing.T) {
	...
	line, _, _ := driveOp(t, "provision", provisionPayload(t, "elasticsearch_repo", repo, nil),
		func(call verbCall) (any, *protoError) {
			...
			case "grep":
				return okExec(), nil
			case "tail":
				return outExec("main ERROR Could not determine local host name\n" +
	...
	if f.OK || f.Error.Code != "restore_failed" ||
		!strings.Contains(f.Error.Message, "Elasticsearch died") {
...
func TestLifecyclePollValue(t *testing.T) {
	tests := []struct {
		name     string
		body     string
		want     string
		...
		{"the node default", fixtureSettings, "876000h", true},
		{"a persistent value overrides the default",
			`{"persistent":{"data_streams.lifecycle.poll_interval":"5m"},"transient":{},"defaults":{"data_streams.lifecycle.poll_interval":"876000h"}}`,
			"5m", true},
		{"a transient value overrides both",
			`{"persistent":{"data_streams.lifecycle.poll_interval":"5m"},"transient":{"data_streams.lifecycle.poll_interval":"1s"},"defaults":{"data_streams.lifecycle.poll_interval":"876000h"}}`,
			"1s", true},
		{"settings without the key", `{"persistent":{},"transient":{},"defaults":{"cluster.name":"x"}}`, "", false},
		{"the simulated sandbox's answer", `1`, "", false},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, answered := lifecyclePollValue([]byte(tc.body))
			if got != tc.want || answered != tc.answered {
				t.Errorf("lifecyclePollValue = %q, %v, want %q, %v", got, answered, tc.want, tc.answered)
			}
		})
	}
}

func TestIlmMode(t *testing.T) {
	if mode, ok := ilmMode([]byte(`{"operation_mode":"STOPPING"}`)); !ok || mode != "STOPPING" {
		t.Errorf("ilmMode = %q, %v", mode, ok)
	}
	for _, body := range []string{`1`, `{}`, `not json`} {
		if _, ok := ilmMode([]byte(body)); ok {
			t.Errorf("ilmMode(%q) answered; want silence on anything but the API's shape", body)
		}
	}
}

func TestLastErrorLine(t *testing.T) {
	log := []byte("[2026-08-21T10:00:00] [INFO ] starting ...\n" +
