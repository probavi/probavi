...
type resolvedSource struct {
	...
	sizeBytes int64
	// archive reports that the artifact is a zip to unpack rather than a
	// tree to transfer.
	archive bool
...
//
//	elasticsearch_repo_zip — path is one zip archive of an fs snapshot
//	                         repository, the repository at the root or
//	                         under one wrapping directory
//	elasticsearch_repo     — path is one fs snapshot repository directory
//
// The archive form is zip rather than tar because of what the verified
// images carry: the 9.x official image ships no tar at all (measured —
// UBI-based, 224 binaries, `unzip` among them and `tar` not), and an
// archive kind that unpacks on one verified line but not the other would
// be a claim this repository cannot keep.
//
...
// is that claim (epoch milliseconds, read through the engine).
func resolveSource(kind, path string) (*resolvedSource, *protoError) {
	switch kind {
	case "elasticsearch_repo_zip":
		return resolveZip(path)
	case "elasticsearch_repo":
		return resolveRepoDir(path)
	default:
		return nil, protoErr("unsupported_source", false,
			"unsupported source kind: %s (supported: elasticsearch_repo_zip, elasticsearch_repo)", kind)
	}
}

// resolveZip vets an archive artifact with what the host can read out of
// it, and falls silent where the bytes are not a zip: the sandbox
// extraction is then the authority (metadata is a bonus, verdicts are
// not guesses).
func resolveZip(path string) (*resolvedSource, *protoError) {
	...
	case info.IsDir():
		return nil, protoErr("invalid_request", false,
			"source path %s is a directory; use kind elasticsearch_repo for a repository directory", path)
	}
	...
	src := &resolvedSource{path: path, checksum: checksum, sizeBytes: info.Size(), archive: true}
	if ok {
		src.census = census
	}
	return src, nil
...
func resolveRepoDir(dir string) (*resolvedSource, *protoError) {
	...
	case !info.IsDir():
		return nil, protoErr("invalid_request", false,
			"source path %s is a file; use kind elasticsearch_repo_zip for zip archives", dir)
	}
