...
func TestResolveSourceKindGates(t *testing.T) {
	...
	archive := treeToZip(t, repo, filepath.Join(base, "repo.zip"), "")
	...
	tests := []struct {
		...
		{"unknown kind", "elasticsearch_backup", repo, "unsupported_source"},
		{"missing archive", "elasticsearch_repo_zip", filepath.Join(base, "gone.zip"), "source_not_found"},
		{"missing directory", "elasticsearch_repo", filepath.Join(base, "gone"), "source_not_found"},
		{"a directory for the zip kind", "elasticsearch_repo_zip", repo, "invalid_request"},
		{"a file for the directory kind", "elasticsearch_repo", archive, "invalid_request"},
	}
...
func TestResolveZipCarriesTheCensus(t *testing.T) {
	repo := writeRepo(t, t.TempDir())
	path := treeToZip(t, repo, filepath.Join(t.TempDir(), "repo.zip"), "")
	src, perr := resolveSource("elasticsearch_repo_zip", path)
	if perr != nil {
		t.Fatalf("resolveSource: %+v", perr)
	}
	if !src.archive || !strings.HasPrefix(src.checksum, "sha256:") || src.sizeBytes <= 0 {
...
func TestResolveZipStaysSilentOnOpaqueBytes(t *testing.T) {
...
func TestResolveRepoDirVetsAndMeasures(t *testing.T) {
	repo := writeRepo(t, t.TempDir())
	src, perr := resolveSource("elasticsearch_repo", repo)
	if perr != nil {
		t.Fatalf("resolveSource: %+v", perr)
	}
	if src.archive || !strings.HasPrefix(src.checksum, "sha256:") || src.sizeBytes <= 0 {
