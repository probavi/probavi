...
// couchbackup_dir restores. File time is the only fact this adapter has to
...
// writeCandidate lays down one candidate couchbackup file, backdated.
func writeCandidate(t *testing.T, base, name string, age time.Duration) string {
	t.Helper()
	path := writeArtifact(t, base, name, backupFixture(3))
...
// chooseBackup is chooseIn bound to the couchbackup candidate filter, the
// way the kind itself calls it.
func chooseBackup(dir string, policy selectPolicy) (string, *protoError) {
	return chooseIn(context.Background(), dir, policy, candidateBackup)
}
...
func TestBackupSelectionAccepts(t *testing.T) {
	if policy, perr := backupSelection("couchbackup_dir", nil); perr != nil || policy != selectNewest {
		t.Fatalf("policy = %q, %+v; want newest where nothing was declared", policy, perr)
	}
	for _, raw := range []string{"newest", "oldest", "random"} {
		policy, perr := backupSelection("couchbackup_dir", map[string]string{selectParam: raw})
	...
	for _, kind := range []string{"couchbackup", "couchdb_data", "couchdb_data_tar"} {
...
func TestBackupSelectionRefuses(t *testing.T) {
	t.Run("a value that is none of the three", func(t *testing.T) {
		_, perr := backupSelection("couchbackup_dir", map[string]string{selectParam: "latest"})
...
func TestSelectionPicksTheEndsOfTheWindow(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got, perr := chooseBackup(base, tc.policy)
		if perr != nil {
...
func TestSelectionTieBreaksDeterministically(t *testing.T) {
	...
	if got, _ := chooseBackup(base, selectNewest); got != z {
		t.Errorf("newest broke the tie at %s, want %s", filepath.Base(got), filepath.Base(z))
	}
	if got, _ := chooseBackup(base, selectOldest); got != a {
...
// whole window and that a file of another kind is still not a candidate.
func TestRandomCoversTheWindow(t *testing.T) {
	base, oldest, middle, newest := selectionWindow(t)
	// A CouchDB data file beside the backups: not a couchbackup, so not a
	// candidate under any policy.
	writeArtifact(t, base, "zz-_dbs.couch", []byte("not a backup\n"))
	seen := map[string]int{}
	for range 200 {
		got, perr := chooseBackup(base, selectRandom)
		if perr != nil {
...
// backup the record does not name. Here it is the *oldest* candidate in
// motion, which the default policy would never have looked at.
func TestTheSettleCheckAppliesUnderEveryPolicy(t *testing.T) {
	...
	_, perr := chooseBackup(base, selectOldest)
	stop()
...
func TestResolveSourceHonoursTheSelection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src, perr := resolveSource(context.Background(), "couchbackup_dir", base, tc.params)
		if perr != nil {
...
func TestResolveSourceRefusesAnImpossibleSelection(t *testing.T) {
	...
	_, perr := resolveSource(context.Background(), "couchbackup", newest,
