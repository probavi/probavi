...
```console
$ docker pull ghcr.io/probavi/probavi:0.7.1
$ docker run --rm ghcr.io/probavi/probavi:0.7.1 version
probavi 0.7.1 linux/amd64
...
$ docker run --rm --user "$(id -u):$(id -g)" -v "$PWD/keys:/keys" \
    ghcr.io/probavi/probavi:0.7.1 evidence keygen --out /keys/probavi.key
...
$ docker run --rm \
    ...
    ghcr.io/probavi/probavi:0.7.1 run --config /etc/probavi/drill.yaml
...
services:
  drill:
    image: ghcr.io/probavi/probavi:0.7.1
    user: "${UID}:${GID}"
    group_add:
...
```console
$ docker build --build-arg VERSION=0.7.1 -t probavi:local .
...
$ docker run --rm --user root \
    ...
    --entrypoint sh ghcr.io/probavi/probavi:0.7.1 \
...
```dockerfile
FROM ghcr.io/probavi/probavi:0.7.1
USER root
