...
```console
$ docker pull ghcr.io/probavi/probavi:0.32.0
$ docker run --rm ghcr.io/probavi/probavi:0.32.0 version
...
$ docker run --rm --user "$(id -u):$(id -g)" -v "$PWD/keys:/keys" \
    ghcr.io/probavi/probavi:0.32.0 evidence keygen --out /keys/probavi.key
...
$ docker run --rm \
    ...
    ghcr.io/probavi/probavi:0.32.0 run --config /etc/probavi/drill.yaml
...
$ docker run --rm --user "$(id -u):$(id -g)" \
    ...
    ghcr.io/probavi/probavi:0.32.0 \
...
services:
  drill:
    image: ghcr.io/probavi/probavi:0.32.0
    user: "${UID}:${GID}"
    group_add:
...
```console
$ docker build --build-arg VERSION=0.32.0 -t probavi:local .
...
$ docker run --rm --user root \
    ...
    --entrypoint sh ghcr.io/probavi/probavi:0.32.0 \
...
```dockerfile
FROM ghcr.io/probavi/probavi:0.32.0
USER root
