...
| [ArangoDB](adapters/arangodb/README.md) | 3.12 | unreleased | `arangodb_dump`, `arangodb_dump_dir`, `arangodb_dump_tar` |
...
| [PostgreSQL](adapters/postgres/README.md) | 14, 15, 16, 17, 18, pgvector 0.8.6-pg17, timescaledb 2.29.1-pg17, postgis 17-3.5 | 0.1.0 | `pgbackrest`, `pgdump`, `pgdump_dir`, `pgdump_with_globals`, `timescaledb_dump`, `timescaledb_dump_dir`, `timescaledb_dump_with_globals` |
...
`probavi adapter conformance` — no container runtime needed. Released as **v0.31.0**: reproducible
...
```console
$ tag=v0.31.0 os=linux arch=amd64
...
Distribution packages are attached to every release — `.deb`, `.rpm` and `.apk` for both architectures, plus a `PKGBUILD` and a Gentoo ebuild that build from source. One package per binary, so `sudo apt install ./probavi_0.31.0_amd64.deb ./probavi-adapter-postgres_0.31.0_amd64.deb` is a working install. There is no Probavi apt or yum repository, on purpose: hosting one means a second long-lived signing key to guard, in a project whose trust proposition is how it handles the first one. [docs/packaging.md](docs/packaging.md) has the per-distribution commands, the dependency rationale, and a first drill from a packaged install.
