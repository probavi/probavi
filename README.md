...
[![ArangoDB](https://img.shields.io/badge/ArangoDB-DDDF72?logo=arangodb&logoColor=333333)](adapters/arangodb/README.md)
...
| [ArangoDB](adapters/arangodb/README.md) | 3.12 | 0.32.0 | `arangodb_dump`, `arangodb_dump_dir`, `arangodb_dump_tar` |
...
| [Chroma](adapters/chroma/README.md) | 1.5.9, 1.4.1 | 0.29.0 | `chroma_data`, `chroma_data_tar` |
...
| [PostgreSQL](adapters/postgres/README.md) | 14, 15, 16, 17, 18, pgvector 0.8.6-pg17, timescaledb 2.29.1-pg17, postgis 17-3.5 | 0.1.0 | `barman`, `pgbackrest`, `pgdump`, `pgdump_dir`, `pgdump_with_globals`, `timescaledb_dump`, `timescaledb_dump_dir`, `timescaledb_dump_with_globals`, `walg` |
...
| [ScyllaDB](adapters/scylladb/README.md) | 2026.3 | 0.31.0 | `scylladb_snapshot`, `scylladb_snapshot_dir`, `scylladb_snapshot_tar` |
...
`probavi adapter conformance` — no container runtime needed. Released as **v0.32.0**: reproducible
...
```console
$ tag=v0.32.0 os=linux arch=amd64
...
Adapters ship for `postgres`, `mysql`, `mariadb`, `mongodb`, `mssql`, `clickhouse`, `etcd`, `redis`, `valkey`, `sqlite`, `duckdb`, `prometheus`, `cassandra`, `opensearch`, `influxdb`, `victoriametrics`, `elasticsearch`, `oracle`, `neo4j`, `solr`, `firebird`, `h2`, `couchdb`, `qdrant`, `weaviate`, `aerospike`, `questdb`, `tdengine`, `chroma`, `iotdb`, `scylladb`, and `arangodb`. Both binaries must sit on the same `PATH`: the core launches the adapter as a child process and finds it by name. Each adapter carries its own version — the one it reports through the protocol and that every evidence record stores as `adapter.version` — which moves independently of the release tag; the compatibility contract between core and adapter is the adapter protocol version, negotiated at handshake. The release notes list both.
...
Distribution packages are attached to every release — `.deb`, `.rpm` and `.apk` for both architectures, plus a `PKGBUILD` and a Gentoo ebuild that build from source. One package per binary, so `sudo apt install ./probavi_0.32.0_amd64.deb ./probavi-adapter-postgres_0.32.0_amd64.deb` is a working install. There is no Probavi apt or yum repository, on purpose: hosting one means a second long-lived signing key to guard, in a project whose trust proposition is how it handles the first one. [docs/packaging.md](docs/packaging.md) has the per-distribution commands, the dependency rationale, and a first drill from a packaged install.
...
- **docker** — containers with `--network none` (loopback only), no published ports, labeled and force-removed with their volumes; an orphan sweep at every drill start reaps leftovers of crashed runs. Podman answers this provider too, through the docker-compatible CLI its `podman-docker` package installs: CI runs the provider's own suite against it rootless, on cgroup v2 with the systemd cgroup manager, where the configured `memory` and `cpus` caps are in force exactly as they are under docker. Adapters are exercised under docker only, so what is verified there is the provider, not every engine image.
...
Drills then run on the remote machine's resources while backups and evidence stay on the host that invokes `probavi`: `put_file` streams the backup bytes through the SSH connection (never a published port), and every container guarantee above — engine image version matching, `--network none`, resource caps, forced destruction — holds exactly as locally. Requirements: key-based SSH to the target and a docker daemon + CLI there. Several drill hosts may safely share one daemon: sandboxes carry a host-scoped label and each host's orphan sweep only ever touches its own containers (upgrade all sharing hosts to ≥ the version that introduced the label before pointing them at a common daemon). That label is derived from the machine id and the hostname together, so it separates hosts that share either one — but a virtual machine cloned without regenerating `/etc/machine-id` keeps both, and two such clones would each treat the other's running sandboxes as their own orphans. systemd requires regenerating it on clone; if yours has not, give the hosts distinct hostnames before sharing a daemon. The SSH endpoint deliberately lives in the environment, not in the drill config — sandbox params are recorded verbatim in signed evidence records, and connection details never belong there. Residual risk to weigh: the remote daemon is effectively root on its machine, and the restored production data briefly lives on that machine's disks — give drills a machine you trust as much as the backup storage itself.
...
Cleanup is layered: `Destroy` kills and stops the slice and removes the workspace on every drill outcome; the next drill from the same host sweeps workspaces whose owner process died; and a target-side transient timer armed at create does the same after a hard deadline (2 h) even if the drill host never comes back. An engine the adapter starts lives in the slice from the `exec` that started it until then — each `exec` runs as a unit that ends with its command and leaves what the command started running, which is why the slice is killed rather than only stopped. `PROBAVI_SSH_TARGET` lives in the environment for the same reason as `DOCKER_HOST` above — connection details never enter drill config or evidence records.
