...
- **docker** — containers with `--network none` (loopback only), no published ports, labeled and force-removed with their volumes; an orphan sweep at every drill start reaps leftovers of crashed runs. Podman answers this provider too, through the docker-compatible CLI its `podman-docker` package installs: CI runs the provider's own suite against it rootless, on cgroup v2 with the systemd cgroup manager, where the configured `memory` and `cpus` caps are in force exactly as they are under docker. Adapters are exercised under docker only, so what is verified there is the provider, not every engine image. One difference to know before you switch: docker resolves an unqualified image name against Docker Hub, podman only through the host's short-name aliases or `unqualified-search-registries` — on a host with neither, spell the registry out (`docker.io/library/postgres:16`). The provider passes your `image` verbatim into the signed record, so it will not rewrite it for you.
