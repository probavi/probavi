...
- ~~Adapter distribution story~~ — **decided 2026-08-17: monorepo (`adapters/`), kept deliberately.** The classic reasons to split repositories do not apply here: adapters are external processes nobody imports (no import path can break), stdlib-only (they cannot pollute `go.mod`), individually versioned behind the adapter-version gate, and conformance-checked one by one — the independence a repository split usually buys is already provided by the protocol. What the single repository uniquely provides grows more valuable with every adapter: the machine-enforced agreement between claims and code — `docs/capabilities.json` generated from the in-tree manifests, the version matrix derived from those same files with tests that fail in both directions when jobs and claims disagree, packaging that refuses an undeclared adapter, and the website reading one file. The costs that do scale (per-PR CI wall clock, maintenance attention) are process costs a split would scatter rather than reduce; the pressure valve short of a split is a separate Go module *inside* the repository (a module boundary is not a repository boundary), and because nothing imports an adapter, later extraction stays cheap — this decision, unusually, is not a one-way door. Revisit only on a named trigger: (a) the pull-request gate's wall clock becomes intolerable even with path-scoped baseline selection, (b) an adapter genuinely needs its own dependency tree, (c) an external maintainer team needs a release cadence this repository cannot give them.
